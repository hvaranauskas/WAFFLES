...
public class KeyValuePairLinkedList<K extends Comparable<K>,V> {
    ...
    // Returns false if key is null or element with given key doesn't exist
    public boolean remove(K key) {
        if (key == null) return false;
        if (head.getValue().getKey() == key) { head = head.getNext(); }
        else {
            ListElement<KeyValuePair<K, V>> ptr = head;
            while (ptr.getNext().getValue().getKey() != key) {
                ptr = ptr.getNext();
